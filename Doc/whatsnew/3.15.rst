...
* :pep:`638`: :ref:`Syntactic macros <whatsnew315-syntactic-macros>`
...
.. _whatsnew315-syntactic-macros:

:pep:`638`: Syntactic Macros
----------------------------

Python now supports syntactic macros, a compile-time metaprogramming facility
that allows transforming the AST before compilation.

A macro invocation uses an identifier followed by ``!``::
...
Expression macros can appear inside expressions::

   y = bar!(x)

Macros are imported at compile time using the ``import!`` and ``from!``
predefined macros::

   import! my_macros as mm
   from! my_macros import my_processor as mp

A macro processor is a 4-tuple ``(func, kind, version, additional_names)``
created by the :func:`macros.macro_processor` decorator::
...
The new :mod:`macros` module provides the expansion infrastructure including
``STMT_MACRO``, ``SIBLING_MACRO``, and ``EXPR_MACRO`` constants.

See :mod:`macros` for full documentation.
